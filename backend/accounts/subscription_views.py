...
from .subscription_serializers import SubscriptionGetSerializer
...
def remove_student_from_class(request, student_id, class_id):
    try:
        ...
        student_class = EventRequest.objects.get(id=class_id)
    ...
    except (Student.DoesNotExist, EventRequest.DoesNotExist):
...
class SubscriptionDeleteAPIView(APIView):
    ...
    def delete(self, request, student_id, class_id):
        ...
        student_class = get_object_or_404(EventRequest, pk=class_id)
        subscription = get_object_or_404(Subscription, student=student, event=student_class)
        subscription.delete()
